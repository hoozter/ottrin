...
use ottrin_core::{
    ...
    FolderKind,
    FileCommand, FileEntry, FileSemantic, ListColumn,
    MillerColumnWidthMode,
    PrivilegedCommand, PrivilegedContext, PrivilegedPayload, PrivilegedRequest, PrivilegedStatus,
    SavedTheme, ThemeCustomization,
    SearchIndexStatus, SearchQuery, SearchResultItem, SearchScope, SearchSort, classify_file,
    platform_case_mode_current,
    SortBy, ThemeMode, ThemePreset, ViewMode, default_home_dir, format_modified, format_size, format_size_units,
};
...
impl Colors {
    ...
    fn for_config(config: &AppConfig) -> Self {
        ...
        if custom.enabled {
            ...
            base.window_border_width = (custom.window_border_width.clamp(0.0, 4.0) * 2.0).round() / 2.0;
            base.accent = Self::from_rgba(custom.accent);
            base.accent_dim = Self::from_rgba([custom.accent[0], custom.accent[1], custom.accent[2], 56]);
            base.selected_bg = Self::from_rgba([custom.accent[0], custom.accent[1], custom.accent[2], 46]);
        ...
        if matches!(config.theme, ThemeMode::Dark | ThemeMode::System) && !custom.enabled {
            let neutral = Self::neutral_text_for_preset(config.theme_preset, base.text_muted, base.text_dim);
...
impl OttrinApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        fonts.font_data.insert(
            ...
            std::sync::Arc::new(egui::FontData::from_static(
                include_bytes!("../assets/MaterialSymbolsFilled.ttf")
            )),
        );
    ...
    fn new_with_config(config: AppConfig) -> Self {
        ...
        std::thread::spawn(move || {
            while let Ok(req) = preview_req_rx.recv() {
                ...
                let _ = preview_res_tx.send(PreviewJobResponse { path: req.path, target_dim: req.target_dim, bytes });
            }
        });

        let colors = Colors::for_config(&config);

        let mut state = AppState { config, ..AppState::default() };
        ...
        app.search_service.update_config(app.state.config.search.clone());
    ...
    fn theme_scope_custom_or_seed(&self, preset: ThemePreset, mode: ThemeMode) -> ThemeCustomization {
        let key = Self::theme_scope_key_for(preset, mode);
        if let Some(saved) = self.state.config.theme_custom_by_preset.get(&key).cloned() {
    ...
    fn sync_theme_editor_hex_inputs(&mut self) {
        ...
        for field in Self::theme_color_field_order() {
            ...
            self.theme_editor
                .color_hex_inputs
                .insert(field, format!("#{:02X}{:02X}{:02X}", rgba[0], rgba[1], rgba[2]));
        }
    }
    ...
    fn generate_save_as_name(&self, base: &str) -> String {
        ...
        if !self.state.config.custom_themes.iter().any(|t| t.name.eq_ignore_ascii_case(&candidate)) {
            return candidate;
        }
        for n in 2..=99 {
            let numbered = format!("{} (edited {})", base.trim(), n);
            if !self.state.config.custom_themes.iter().any(|t| t.name.eq_ignore_ascii_case(&numbered)) {
                return numbered;
            }
        }
        format!("{} (edited)", base.trim())
    ...
    fn do_save_theme_as(&mut self, close_and_apply: bool) {
        ...
        if Self::is_builtin_theme_name(&name) {
            ...
            self.theme_status = Some((
                format!("\"{}\" is a built-in theme — renamed to \"{}\"", name, self.theme_editor.save_as_name),
                false,
                Instant::now() + Duration::from_secs(4),
            ));
            return;
    ...
    fn render_appearance_settings(&mut self, ui: &mut egui::Ui, ctx: &Context, c: Colors) {
        ...
        ui.add_space(10.0);
        ui.label(RichText::new("Theme presets").color(c.text_dim).size(11.0).strong());
        ui.add_space(6.0);
        ...
        Frame::new()
            ...
            .show(ui, |ui| {
                ui.label(RichText::new("Saved themes").color(c.text).size(12.0).strong());
                ui.add_space(6.0);
                if self.state.config.custom_themes.is_empty() {
                    ui.label(RichText::new("No saved custom themes yet.").color(c.text_muted).size(10.5));
                } else {
                    ...
                    for (idx, saved) in self.state.config.custom_themes.iter().enumerate() {
                        ...
                        let card_fill = if is_active { c.panel_raised.gamma_multiply(1.08) } else { c.panel_raised };
                        ...
                        let resp = Frame::new()
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    theme_preview_strip(
                                        ui,
                                        self.preview_colors_for_scope(saved.base_preset, saved.theme_mode),
                                        72.0,
                                        24.0,
                                        c,
                                    );
                                    ...
                                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                        if ui.add(
                                            egui::Button::new(RichText::new(MI_CLEAR).size(14.0).color(c.text_muted))
                                                .frame(false),
                                        ).on_hover_text("Delete theme").clicked() {
                                            remove_idx = Some(idx);
                                        }
                                        if ui.add(
                                            egui::Button::new(RichText::new(MI_EDIT).size(14.0).color(c.text_muted))
                                                .frame(false),
                                        ).on_hover_text("Edit theme").clicked() {
                                            edit_idx = Some(idx);
                                        }
                                    });
                                });
    ...
    fn render_theme_editor_popup(&mut self, ctx: &Context) {
        ...
        if close_requested {
            ...
            ctx.send_viewport_cmd_to(Self::theme_editor_viewport_id(), egui::ViewportCommand::Close);
    ...
    fn render_theme_editor_embedded(&mut self, ctx: &Context) -> bool {
        let c = self.colors;
        let mut close_requested = false;
        let popup_title = format!("Theme - {}", Self::theme_preset_label(self.theme_editor.preset));
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                let content_rect = Rect::from_min_max(
                    body_rect.min,
                    egui::pos2(body_rect.max.x - 14.0, footer_rect.min.y - 8.0),
                );
                ...
                ui.scope_builder(UiBuilder::new().max_rect(content_rect), |ui| {
                    ScrollArea::vertical().show(ui, |ui| {
                        ...
                        let button_radius = self.theme_editor.custom.button_radius.clamp(0.0, 14.0) as u8;
                    ...
                    let color_row = |ui: &mut egui::Ui, app: &mut Self, field: ThemeColorField, c: Colors| -> bool {
                        ...
                        ThemeEditorTab::MainInterface => {
                            ...
                            ui.columns(2, |cols| {
                                cols[0].vertical(|ui| {
                                    let mut body = |ui: &mut egui::Ui| {
                                        let width = ui.available_width().clamp(360.0, 520.0);
                                        let _ = theme_preset_card_with_size(
                                            ui,
                                            c,
                                            preview,
                                            MI_EDIT,
                                    ...
                                    section_card_fixed(
                                        ...
                                        top_pair_h,
                                        "Live preview",
                                        "A compact snapshot of the window chrome you are editing right now.",
                                        &mut body,
                                    );
                                });
                                cols[1].vertical(|ui| {
                                    ...
                                    section_card_fixed(
                                        ...
                                        top_pair_h,
                                        "Quick tuning",
                                        "Broad controls for the whole interface before you fine-tune individual colors.",
                                        &mut body,
                                    );
                                });
                            });
                            ...
                            ui.columns(2, |cols| {
                                cols[0].vertical(|ui| {
                                    let mut body = |ui: &mut egui::Ui| {
                                        changed |= color_row(ui, self, ThemeColorField::Background, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::Panel, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::Titlebar, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::Border, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::WindowBorder, c);
                                    };
                                    section_card(
                                        ui,
                                        "Surfaces",
                                        "These colors define the base layers of the window and how clearly they separate.",
                                        &mut body,
                                    );
                                    ui.add_space(10.0);
                                    let mut body = |ui: &mut egui::Ui| {
                                    ...
                                    section_card(
                                        ui,
                                        "Shape",
                                        "Corner softness for the chrome and controls.",
                                        &mut body,
                                    );
                                });
                                cols[1].vertical(|ui| {
                                    let mut body = |ui: &mut egui::Ui| {
                                        changed |= color_row(ui, self, ThemeColorField::Toolbar, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::SmartPanel, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::Accent, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::ButtonBg, c);
                                        ui.add_space(6.0);
                                        changed |= color_row(ui, self, ThemeColorField::ButtonText, c);
                                    };
                                    section_card(
                                        ui,
                                        "Navigation and actions",
                                        "Tabs, toolbars, smart panels, selection accents, and filled buttons.",
                                        &mut body,
                                    );
                                    ui.add_space(10.0);
                                    let mut body = |ui: &mut egui::Ui| {
                                        let btn_bg = Colors::from_rgba(self.theme_editor.custom.button_bg);
                                        let btn_fg = Colors::from_rgba(self.theme_editor.custom.button_text);
                                        let row_bg = mix_color(preview.bg, preview.panel_raised, 0.40);
                                        let mut style = ui.style_mut().clone();
                                        style.visuals.widgets.inactive.bg_fill = btn_bg;
                                        style.visuals.widgets.inactive.fg_stroke = Stroke::new(1.0, btn_fg);
                                        style.visuals.widgets.inactive.bg_stroke = Stroke::NONE;
                                        style.visuals.widgets.inactive.corner_radius =
                                            egui::CornerRadius::same(button_radius);
                                        Frame::new()
                                            .fill(row_bg)
                                            .stroke(Stroke::new(1.0, mix_color(preview.border, row_bg, 0.38)))
                                            .corner_radius(section_radius)
                                            .inner_margin(egui::Margin::symmetric(12, 10))
                                            .show(ui, |ui| {
                                                ui.horizontal(|ui| {
                                                    ui.label(RichText::new("Selected row").color(preview.text).size(11.5));
                                                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                                        ui.scope(|ui| {
                                                            ui.set_style(style);
                                                            let _ = ui.add(egui::Button::new(RichText::new("Button").color(btn_fg)));
                                                        });
                                                    });
                                                });
                                            });
                                    ...
                                    section_card(
                                        ui,
                                        "Action preview",
                                        "A quick read on whether your button and selection colors still feel balanced.",
                                        &mut body,
                                    );
                                });
                            });
            ...
            if let Some(field) = self.theme_editor.color_picker_field {
                egui::Window::new("theme_editor_color_picker_window")
                    ...
                    .show(ctx, |ui| {
                        ...
                        ui.add_space(8.0);
                        let mut rgba = Self::theme_color_field_value(&self.theme_editor.custom, field);
                        ...
                        if egui::color_picker::color_picker_color32(
                            ...
                            *Self::theme_color_field_value_mut(&mut self.theme_editor.custom, field) = rgba;
                            self.reanchor_theme_editor_quick_from_current();
                            self.apply_theme_editor_live_preview(ctx);
                        }
                        ui.add_space(6.0);
                        ui.horizontal(|ui| {
                            ...
                            if ui
                                ...
                                .changed()
                            {
                                self.theme_editor.color_hex_inputs.insert(field, hex.clone());
                            ...
                            if ui
                                .add_enabled(!self.theme_editor.color_clipboard_hex.is_empty(), egui::Button::new("Paste"))
                                .clicked()
                            {
    ...
    fn is_bookmarked(&self, path: &Path) -> bool {
        self.state.config.bookmarks.iter().any(|(_, _, p)| p == path)
    }
    ...
    fn add_bookmark_for_path(&mut self, path: PathBuf) {
        ...
        self.state.config.bookmarks.push((MI_FOLDER.to_string(), name, path));
    }
    ...
    fn refresh_tab(&mut self, tab_id: u64, dir: PathBuf) {
        ...
        if let Some(ui_state) = self.tab_ui.get_mut(&tab_id) {
            ...
            while let Some(parent) = cursor {
                ui_state.selection_memory.insert(parent.clone(), child.clone());
                child = parent.clone();
        ...
        while let Ok(resp) = self.listing_rx.try_recv() {
            ...
            if matches!(resp.key, ListingKey::TabMain(_)) {
                ...
                if !ui_state.main.entries.is_empty() {
                    ...
                    let idx = desired
                        .as_ref()
                        .and_then(|path| ui_state.main.entries.iter().position(|e| &e.path == path));
                    if let Some(i) = idx {
    ...
    fn poll_op_results(&mut self) {
        while let Ok(res) = self.op_rx.try_recv() {
            ...
            if let Some(tab_id) = res.tab_id
                && let Some(tab) = self.state.tab_by_id(tab_id)
            {
    ...
    fn remember_main_selection(&mut self, tab_id: u64) {
        ...
        if let Some((dir, path)) = selected {
            self.tab_ui.entry(tab_id).or_default().selection_memory.insert(dir, path);
        }
    }
    ...
    fn set_main_selection(&mut self, tab_id: u64, idx: usize) {
        let selected = {
            ...
            ui.main.entries.get(idx).map(|e| (ui.main.dir.clone(), e.path.clone()))
        };
        if let Some((dir, path)) = selected {
            self.tab_ui.entry(tab_id).or_default().selection_memory.insert(dir, path);
        }
    ...
    fn build_miller_model(&mut self, tab_id: u64, _pane_count: usize) -> (Vec<MillerColumnModel>, usize) {
        let current_dir = self.state
            .tab_by_id(tab_id)
            .map(|t| t.current_dir.clone())
            .unwrap_or_else(default_home_dir);
        let sort_cfg = self.state
            .tab_by_id(tab_id)
            .map(|t| t.sort)
            .unwrap_or_default();
        ...
        let max_depth = self.tab_ui.entry(tab_id).or_default().miller_preview_depth.max(1);
    ...
    fn navigate_active_back(&mut self) {
        let tab_id = self.state.active_tab().id;
        let before = self.state.active_tab().current_dir.clone();
        self.remember_main_selection(tab_id);
    ...
    fn handle_keyboard(&mut self, ctx: &Context) {
        ...
        if self.sidebar == SidebarSection::Search && !self.command_frame.visible && self.search_ui.input_focused {
        ...
        if self.sidebar == SidebarSection::Search && !self.command_frame.visible && !self.search_ui.input_focused {
            let mut consumed = false;
            ctx.input_mut(|input| {
                if input.consume_key(egui::Modifiers::NONE, Key::ArrowDown) && !self.search_ui.results.is_empty() {
                    consumed = true;
                    let cur = self.search_ui.selected.unwrap_or(0);
                    self.search_ui.selected = Some((cur + 1).min(self.search_ui.results.len() - 1));
                    self.search_ui.scroll_to_selected = true;
                }
                if input.consume_key(egui::Modifiers::NONE, Key::ArrowUp) && !self.search_ui.results.is_empty() {
                    consumed = true;
                    let cur = self.search_ui.selected.unwrap_or(0);
                    self.search_ui.selected = Some(cur.saturating_sub(1));
                    self.search_ui.scroll_to_selected = true;
                ...
                if is_miller {
                    ...
                    if !cols.is_empty() {
                        ...
                        if !col.entries.is_empty() {
                            ...
                            self.set_miller_selection(tab_id, &col.dir, &col.entries, next, focus_col == current_col);
                            let ui_state = self.tab_ui.entry(tab_id).or_default();
                ...
                if is_miller {
                    ...
                    if !cols.is_empty() {
                        ...
                        if !col.entries.is_empty() {
                            ...
                            self.set_miller_selection(tab_id, &col.dir, &col.entries, prev, focus_col == current_col);
                            let ui_state = self.tab_ui.entry(tab_id).or_default();
                            ui_state.miller_scroll_selected = true;
                            ui_state.miller_preview_depth = 1;
                            if focus_col < current_col {
                        ...
                        if focus_col < current_col && focus_col + 1 < cols.len() {
                            ...
                            let new_dir = target_col.dir.clone();
                            {
                                    ...
                                    && let Some(sel_entry) = target_col.entries.get(sel_idx)
                                {
                                    ...
                                    let remembered = ui_state.selection_memory
                                        .get(&new_dir).cloned();
                            ...
                            if matches!(entry.kind, EntryKind::Directory | EntryKind::Symlink) {
                                ...
                                self.tab_ui.entry(tab_id).or_default().miller_focus_dir = Some(cols[focus_col + 1].dir.clone());
                            } else {
                    ...
                    if !cols.is_empty() {
                        ...
                        if focus_col > 0 {
                            ...
                            let moved_from_dir = cols[focus_col].dir.clone();
                            {
                                let ui_state = self.tab_ui.entry(tab_id).or_default();
                                ui_state.miller_focus_dir = Some(new_dir.clone());
                                ui_state.miller_preview_depth = ui_state.miller_preview_depth.max(1) + 1;
            ...
            if input.key_pressed(Key::Escape) {
                if self.sidebar == SidebarSection::Search {
                    ...
                    self.tab_ui
                        .entry(tab_id)
                        .or_default()
                        .miller_focus_hint = Some(MillerFocusHint::CurrentDir);
                }
        ...
        let interacted_elsewhere = ctx.input(|i| {
            i.pointer.any_pressed()
                ...
                || i.events.iter().any(|ev| {
                    matches!(ev, egui::Event::Text(t) if !t.trim().is_empty())
                })
        });
    ...
    fn middle_ellipsis(ui: &egui::Ui, text: &str, max_width: f32, font: &FontId) -> String {
        ...
        let text_width = ui.fonts_mut(|f| f.layout_no_wrap(text.to_string(), font.clone(), Color32::WHITE).size().x);
        if text_width <= max_width {
            return text.to_string();
        }
        let ellipsis_width = ui.fonts_mut(|f| f.layout_no_wrap(ellipsis.to_string(), font.clone(), Color32::WHITE).size().x);
        ...
        while lo <= hi {
            ...
            let width = ui.fonts_mut(|f| f.layout_no_wrap(candidate.clone(), font.clone(), Color32::WHITE).size().x);
            if width <= max_width {
                best = candidate;
                lo = mid + 1;
            } else {
    ...
    fn active_selected_entry(&mut self) -> Option<FileEntry> {
        let tab_id = self.state.active_tab().id;
        if matches!(self.state.active_tab().view_mode, ViewMode::Miller) {
            let (cols, current_col) = self.build_miller_model(tab_id, self.miller_col_count);
            if cols.is_empty() {
                return None;
            }
        ...
        let ui = self.tab_ui.entry(tab_id).or_default();
        ui.main_sel.and_then(|idx| ui.main.entries.get(idx)).cloned()
    }

    fn displayed_path(&mut self) -> PathBuf {
        let fallback = self.state.active_tab().current_dir.clone();
        if !matches!(self.state.active_tab().view_mode, ViewMode::Miller) {
            return fallback;
        }
        match self.active_selected_entry() {
            Some(entry) if matches!(entry.kind, EntryKind::Directory | EntryKind::Symlink) => entry.path,
            _ => fallback,
        }
    }
    ...
    fn retry_list_with_privileges(&mut self, tab_id: u64, dir: PathBuf) -> Result<(), String> {
        ...
        let res = self.platform.execute_privileged(&req).map_err(|e| e.to_string())?;
        match res.status {
            PrivilegedStatus::Success => {
                ...
                self.tab_ui.entry(tab_id).or_default().miller_cache.insert(dir.clone(), Ok(entries.clone()));
                let sort_cfg = self.state.tab_by_id(tab_id).map(|t| t.sort).unwrap_or_default();
            ...
            PrivilegedStatus::Denied => {
                let msg = res.message.unwrap_or_else(|| "Permission denied".to_string());
                self.privileged_status = Some(PrivilegedUiStatus {
            ...
            PrivilegedStatus::Failed => {
                let msg = res.message.unwrap_or_else(|| "Privileged operation failed".to_string());
                self.privileged_status = Some(PrivilegedUiStatus {
                    kind: PrivilegedUiKind::Error,
                    text: msg.clone(),
                    since: Instant::now(),
    ...
    fn run_file_op(&mut self, cmd: FileCommand, refresh_tab: Option<u64>) {
        ...
        std::thread::spawn(move || {
            ...
            let _ = op_tx.send(OpResult { tab_id: refresh_tab, error, retry_cmd: retry });
        });
    }

    fn retry_file_op_with_privileges(&mut self, cmd: FileCommand, refresh_tab: Option<u64>) -> Result<(), String> {
        self.privileged_status = Some(PrivilegedUiStatus {
            kind: PrivilegedUiKind::InProgress,
            text: "Requesting administrator access…".to_string(),
            since: Instant::now(),
    ...
    fn run_search_query(&mut self) {
        ...
        if self.search_ui.last_error.is_none() {
            ...
            self.search_ui.selected = previously_selected_path
                .and_then(|path| self.search_ui.results.iter().position(|it| it.path == path))
                .or(if self.search_ui.results.is_empty() { None } else { Some(0) });
            self.search_ui.scroll_to_selected = true;
        }
    }

    fn open_search_selected(&mut self) {
        let entry = self.search_ui.selected
            .and_then(|i| self.search_ui.results.get(i))
            .cloned();
    ...
    fn render_empty_folder_state(
        ...
        ui.with_layout(Layout::top_down(Align::Center), |ui| {
            ui.label(RichText::new("This folder is empty").color(c.text_muted).size(12.0));
            if show_hint {
                ui.add_space(8.0);
                ui.label(
    ...
    fn render_tab_row(&mut self, ctx: &Context) {
        ...
        let panel_resp = egui::TopBottomPanel::top("tab_row")
            ...
            .frame(Frame::new().fill(titlebar_bg).inner_margin(egui::Margin::symmetric(6, 1)))
            .show(ctx, |ui| {
                ...
                ui.with_layout(Layout::left_to_right(Align::Center), |ui| {
                    ...
                    for i in 0..n_tabs {
                        ...
                        let tab_fill = if is_active { active_tab_bg } else { Color32::TRANSPARENT };
                        let text_color = if is_active { c.text_muted } else { inactive_text };
                        ...
                        let frame = Frame::new()
                            ...
                            .inner_margin(egui::Margin { left: 14, right: 6, top: 4, bottom: 4 });
                        ...
                        let tab_click_rect = if let Some(close_rect) = close_rect {
                            ...
                            Rect::from_min_max(
                                tab_rect.min,
                                egui::pos2((close_rect.min.x - 6.0).max(tab_rect.min.x), tab_rect.max.y),
                            )
                        } else {
                            tab_rect
                        };
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ...
                        let max_btn = wm_btn(ui, WmBtn::Maximize, &c).on_hover_text(if is_max {"Restore"} else {"Maximize"});
    ...
    fn render_nav_row(&mut self, ctx: &Context) {
        ...
        egui::TopBottomPanel::top("nav_row")
            .exact_height(CUSTOM_TITLEBAR_HEIGHT)
            .frame(Frame::new().fill(c.toolbar_bg).inner_margin(egui::Margin::symmetric(8, 0)))
            .show(ctx, |ui| {
                ...
                ui.with_layout(Layout::left_to_right(Align::Center), |ui| {
                    ...
                    let back_col = if can_back { c.text_muted } else { c.text_muted.gamma_multiply(0.62) };
                    let fwd_col  = if can_fwd  { c.text_muted } else { c.text_muted.gamma_multiply(0.62) };
                    let up_col   = if can_up   { c.text_muted } else { c.text_muted.gamma_multiply(0.62) };
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ...
                        if ui.add(egui::Button::new(RichText::new(view_icon).size(20.0).color(c.text_muted)).frame(false).min_size(Vec2::splat(30.0))).on_hover_text(view_tip).clicked() {
                            let next = match view {
    ...
    fn render_bookmarks_row(&mut self, ctx: &Context) {
        ...
        egui::TopBottomPanel::top("bookmarks_row")
            ...
            .show(ctx, |ui| {
                ui.painter().hline(ui.max_rect().x_range(), ui.max_rect().bottom(), Stroke::new(1.0, mix_color(c.border, c.toolbar_bg, 0.5)));
                ...
                ui.with_layout(Layout::left_to_right(Align::Center), |ui| {
                    ...
                    for (idx, (icon, name, path)) in bookmarks.iter().enumerate() {
                        ...
                        let (text_col, bg_col) = if is_here {
                            (
                                c.text_muted,
                                c.selected_bg.gamma_multiply(0.62),
                            )
                        } else {
    ...
    fn render_address_bar(&mut self, ui: &mut egui::Ui) {
        ...
        if self.address_bar.editing {
            ...
            if bar_interact.clicked() {
                let path_str = self.displayed_path().display().to_string();
    ...
    fn compute_smart_panel_layout(&mut self, avail: Rect) -> (Rect, Rect) {
        ...
        let sidebar_inline = is_miller
            && matches!(self.sidebar, SidebarSection::Info | SidebarSection::None);
        ...
        let panel_width = if content_w > 0.0 { strip_w + content_w } else { strip_w };
        ...
        let file_rect = Rect::from_min_max(
            avail.min,
            egui::pos2(panel_rect.min.x, avail.max.y),
        );
    ...
    fn render_status_bar(&mut self, ctx: &Context) {
        ...
        egui::TopBottomPanel::bottom("status_bar")
            ...
            .frame(Frame::new().fill(bottom_bar_bg).inner_margin(egui::Margin::symmetric(12, 0)))
            .show(ctx, |ui| {
                ui.set_height(STATUS_BAR_HEIGHT);
                ui.painter().hline(ui.max_rect().x_range(), ui.max_rect().top(), Stroke::new(1.0, mix_color(c.border, bottom_bar_bg, 0.5)));
                ui.with_layout(Layout::left_to_right(Align::Center), |ui| {
                    // Command frame toggle — LEFT side (bar opens at the bottom-left)
                    let cf_color = if self.command_frame.visible { c.accent_dim } else { bottom_text };
                    let cf_resp = ui.add(
                    ...
                    if let Some(msg) = &self.status_message {
                        ...
                        ui.label(RichText::new(format!("· {}", msg.text)).color(col).size(11.0));
                    }
                        ...
                        match &self.privileged_status {
                            ...
                            None => {
                                let (icon, col, tip) = match &self.privileged_availability {
                                    PrivilegedAvailability::Ready => (MI_LOCK, bottom_text, "Integrated privilege management ready"),
                                    PrivilegedAvailability::Misconfigured(msg) => (MI_LOCK, c.error, msg.as_str()),
                                    PrivilegedAvailability::Unsupported(msg) => (MI_LOCK, bottom_text, msg.as_str()),
                                };
                                let resp = ui.add(
    ...
    fn render_search_sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        let border_col = if self.search_ui.input_focused { c.accent } else { c.border };
        let border_w = if self.search_ui.input_focused { 1.5 } else { 1.0 };
        Frame::new()
            .fill(c.panel_raised)
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    let icon_col = if self.search_ui.input_focused { c.accent } else { c.text_muted };
                    ...
                    if resp.changed() { self.run_search_query(); }
                });
            });
        ...
        ui.horizontal(|ui| {
            ...
            for (label, is_active, scope) in [
                ...
                let fill = if is_active { c.selected_bg } else { Color32::TRANSPARENT };
                ...
                if btn.clicked() { self.search_ui.scope = scope; self.run_search_query(); }
            }

            // Settings gear — far right, subtle
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ui.add_space(4.0);
                let gear = ui.add(egui::Label::new(
                    RichText::new(MI_SETTINGS).size(13.0).color(c.text_muted)
                ).sense(Sense::click()));
                gear.clone().on_hover_text("Search settings");
                if gear.hovered() { ui.ctx().set_cursor_icon(egui::CursorIcon::PointingHand); }
                if gear.clicked() { self.show_settings = true; self.settings_tab = SettingsTab::Search; }
            });
        });
        ...
        if self.search_ui.query.is_empty() {
            ...
            ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ...
                    ui.label(RichText::new("Start typing to search").color(c.text).size(14.0).strong());
                    ui.add_space(14.0);
                    ui.label(RichText::new("Search across all indexed locations").color(c.text_muted).size(11.5));
                    ui.add_space(4.0);
                    ui.label(RichText::new("Use multiple words to narrow results").color(c.text_muted).size(11.5));
                    ...
                    match diag.status {
                        SearchIndexStatus::Indexing => {
                            ...
                            ui.horizontal(|ui| {
                                ui.label(RichText::new(format!("Indexing files… ({}%)", pct))
                        ...
                        SearchIndexStatus::Unavailable => {
                            ui.label(RichText::new("Search unavailable").color(c.error).size(11.0));
                        }
                    }
                },
            );
        ...
        // ── Results header (count + error) ───────────────────────────────
        {
            ...
            ui.horizontal(|ui| {
                let count_label = if n == 1 { "1 result".to_string() } else { format!("{n} results") };
                ...
                if matches!(diag.status, SearchIndexStatus::Indexing) {
                    ...
                    ui.label(RichText::new(format!("· indexing {}%", pct)).color(c.accent).size(10.5));
...

        {
                ...
                let mut table = TableBuilder::new(ui)
                    ...
                    .max_scroll_height(results_height - 18.0)
                    .min_scrolled_height((results_height - 18.0).max(120.0))
                ...
                table
                    .header(22.0, |mut header| {
                        let mut header_col =
                            |header: &mut egui_extras::TableRow<'_, '_>, label: &str, sort: SearchSort, right_align: bool| {
                                header.col(|ui| {
                    ...
                    .body(|mut body| {
                        for (i, item) in self.search_ui.results.iter().enumerate() {
                            ...
                            body.row(24.0, |mut row| {
                                ...
                                row.col(|ui| {
                                    ui.horizontal(|ui| {
                                        ui.label(RichText::new(icon).size(11.5).color(style.icon_color));
                                        ui.label(
                                ...
                                row.col(|ui| {
                                    ui.label(
                                        RichText::new(truncate_name(&item.parent_path.display().to_string(), folder_cap))
                                            .size(10.5)
                                            .color(c.text_muted),
                                    );
                                });
                                row.col(|ui| {
                                    ui.label(RichText::new(modified_text).size(10.5).color(c.text_muted));
                                });
                                row.col(|ui| {
                                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                        ui.label(RichText::new(size_text).size(10.5).color(c.text_muted));
                                    });
                                });
                                ...
                                if ui.button(RichText::new("Open").color(c.text_muted)).clicked() {
                                    open_entry_now = Some((path.clone(), item.kind));
                                    ui.close();
                                }
                                if ui.button(RichText::new("Open Folder Location").color(c.text_muted)).clicked() {
                                    open_parent = Some(parent.clone());
                                    ui.close();
                                }
                                if ui.button(RichText::new("Copy File Path").color(c.text_muted)).clicked() {
                                ...
                                let copy_target_btn = ui
                                    .add_enabled(
                                        ...
                                        egui::Button::new(RichText::new("Copy to Drop Folder").color(c.text_muted)),
                                    )
                                ...
                                let move_target_btn = ui
                                    .add_enabled(
                                        !target_missing && !same_as_target,
                                        egui::Button::new(RichText::new("Move to Drop Folder").color(c.text_muted)),
                                    )
                                    .on_hover_text(if target_missing {
                                        "No drop folder set"
                                    } else if same_as_target {
                                ...
                                let copy_cur_btn = ui
                                    .add_enabled(
                                        !same_as_current,
                                        egui::Button::new(RichText::new("Copy to Current Folder").color(c.text_muted)),
                                    )
                                ...
                                let move_cur_btn = ui
                                    .add_enabled(
                                        !same_as_current,
                                        egui::Button::new(RichText::new("Move to Current Folder").color(c.text_muted)),
                                    )
                                    .on_hover_text(if same_as_current {
                                        "Already in destination"
                                    } else {
                        ...
                        if ui.add(
                            ...
                            match self.retry_file_op_with_privileges(cmd, tab_id) {
                                Ok(()) => {
                                    self.pending_privileged_retry = None;
                                    self.command_frame.error = None;
                                    self.command_frame.message =
                                        Some("Operation completed with administrator privileges.".to_string());
                                }
                                Err(e) => {
    ...
    fn entry_context_menu(
        ...
        response.context_menu(|ui| {
            ...
            if ui.button(RichText::new("Open Folder Location").color(menu_col)).clicked() {
                *open_parent = Some(parent.clone());
                ui.close();
            }
            if ui.button(RichText::new("Copy File Path").color(menu_col)).clicked() {
                ui.ctx().copy_text(path.display().to_string());
            ...
            if is_dir_like {
                if !is_bookmarked {
                    if ui.button(RichText::new("Add to Shortcuts").color(menu_col)).clicked() {
                        *add_bookmark = Some(path.clone());
                        self.set_status_message("Added to shortcuts", true, 3);
                        ui.close();
                    }
                } else if ui.button(RichText::new("Remove from Shortcuts").color(menu_col)).clicked() {
    ...
    fn render_file_pane(&mut self, ctx: &Context) {
        ...
        let tab_id = self.state.active_tab().id;
        let current_sort = self.state.tab_by_id(tab_id).map(|t| t.sort).unwrap_or_default();
        ...
        // View controls (sorting + per-view controls)
        egui::TopBottomPanel::bottom("view_controls_bar")
            .exact_height(VIEW_CONTROLS_BAR_HEIGHT)
            .frame(Frame::new().fill(bottom_bar_bg).inner_margin(egui::Margin::symmetric(10, 0)))
            .show(ctx, |ui| {
                ui.painter().hline(ui.max_rect().x_range(), ui.max_rect().top(), Stroke::new(1.0, mix_color(c.border, bottom_bar_bg, 0.5)));
                ui.with_layout(Layout::left_to_right(Align::Center), |ui| {
                    ...
                    egui::ComboBox::from_id_salt(("sort_by", tab_id))
                        .selected_text(RichText::new(match sort_by {
                            SortBy::Name => "Name",
                            SortBy::Kind => "Type",
                            SortBy::Size => "Size",
                            SortBy::Modified => "Modified",
                        }).color(bottom_text).size(11.5))
                        .width(90.0)
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut sort_by, SortBy::Name, RichText::new("Name").color(bottom_text));
                            ui.selectable_value(&mut sort_by, SortBy::Kind, RichText::new("Type").color(bottom_text));
                            ui.selectable_value(&mut sort_by, SortBy::Size, RichText::new("Size").color(bottom_text));
                            ui.selectable_value(&mut sort_by, SortBy::Modified, RichText::new("Modified").color(bottom_text));
                        });
    ...
    fn render_view_scale_control(&mut self, ui: &mut egui::Ui, view_mode: ViewMode, current_dir: &Path, c: Colors) {
        ...
        ui.label(RichText::new(format!("{:.0}%", scale * 100.0)).size(11.0).color(control_text));
        ui.add_space(8.0);
        if ui.add(
            egui::Button::new(
                RichText::new(MI_SETTINGS_BACKUP_RESTORE).size(15.0).color(control_text)
            )
            .frame(false)
            .min_size(Vec2::new(20.0, 20.0))
        ).on_hover_text("Reset current view size").clicked() {
        ...
        ui.painter().circle_filled(knob_center, 6.0, mix_color(c.text_muted, track_fill, 0.6));
        ui.painter().circle_stroke(knob_center, 6.0, Stroke::new(1.0, c.bg));
        ...
        {
            let t = ((pointer.x - (track_rect.left() + 9.0)) / (track_rect.width() - 18.0)).clamp(0.0, 1.0);
    ...
    fn render_miller(&mut self, ui: &mut egui::Ui, tab_id: u64) {
        ...
        let is_up_down = ui.input(|i| {
            i.key_pressed(Key::ArrowUp) || i.key_pressed(Key::ArrowDown)
        }) && !cmd_open;
        let scroll_left_nav = ui.input(|i| {
            i.key_pressed(Key::ArrowLeft) || i.key_pressed(Key::Backspace)
        }) && !cmd_open;
        let scroll_right_nav = ui.input(|i| {
            i.key_pressed(Key::ArrowRight) || i.key_pressed(Key::Enter)
        }) && !cmd_open;
        ...
        let focus_selected = cols.get(focus_col)
        ...
        let sa_output = sa.show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    let preview_depth = self.tab_ui.entry(tab_id)
                        .or_default().miller_preview_depth.max(1);
                    for (col_idx, col) in cols.iter().enumerate() {
                        let mut width = if col_idx > current_col {
                            ...
                            if let Some(saved) = self.state.config.folder_column_widths.get(&key).copied() {
                                ...
                                ScrollArea::vertical()
                                    ...
                                    .show(ui, |ui| {
                                        ...
                                        if let Some(err) = &col.error {
                                            ...
                                            if retry {
                                                let _ = self.retry_list_with_privileges(tab_id, col.dir.clone());
                                            }
                                            return;
                                        }
                                        if col.entries.is_empty() {
                                        ...
                                        for (i, entry) in col.entries.iter().enumerate() {
                                            ...
                                            if scroll_selected && col_idx == focus_col && selected {
                                                ...
                                                ui.scroll_to_rect_animation(resp.rect, Some(egui::Align::Center), row_smooth);
                                            }
                                            if resp.clicked() {
                                                let is_current = col_idx == current_col;
                                                self.set_miller_selection(tab_id, &col.dir, &col.entries, i, is_current);
                                                {
                                                    let ui_state = self.tab_ui.entry(tab_id).or_default();
                        ...
                        if col_idx == focus_col {
                            ...
                            if scroll_right_nav {
                                ...
                                if needed_right > clip.right() {
                                    ...
                                    ui.scroll_to_rect_animation(padded, Some(egui::Align::RIGHT), smooth);
                                }
                            }
                        }
                        ...
                        {
                            ui.painter().vline(rect.max.x, rect.y_range(), Stroke::new(1.0, c.border));
                    ...
                    let is_file_preview = focus_selected.as_ref()
                    ...
                    // file/folder preview never shrinks the total width.
                    {
                        ...
                        if remaining > 1.0 {
                            ...
                            for fi in 0..n_fill {
                                ...
                                if fi < n_fill - 1 {
                                    ui.painter().vline(r.max.x, r.y_range(), Stroke::new(1.0, c.border));
                                }
                            }
                        }
                    }
    ...
    fn render_miller_file_preview(
        ...
        let inner = ui.allocate_ui_with_layout(
            ...
            |ui| {
                ...
                let details_open = ui.ctx().data_mut(|d| {
                    *d.get_persisted_mut_or_default::<bool>(details_id)
                });
                ...
                let style = style_for_entry(
                    ...
                    EntryVisualState {
                        ...
                        hovered: false, opened: false, symlink_dir: false,
                    },
                    &c,
                    &self.state.config,
                );
                ...
                match preview.kind {
                    PreviewKind::Image => {
                        if let Some(img) = self.cached_image_preview_widget(
                            &entry.path, max_preview_w, max_preview_h, 8,
                        ) {
                            preview_widget = Some(img);
                        }
                    }
                    PreviewKind::Text => {
                        let is_svg = entry.path.extension()
                            .and_then(|e| e.to_str())
                            .map(|e| e.eq_ignore_ascii_case("svg"))
                            .unwrap_or(false);
                        if is_svg {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.vertical(|ui| {
                                ...
                                let meta_row = |ui: &mut egui::Ui, label: &str, value: &str| {
                                    ui.horizontal(|ui| {
                                        ui.label(RichText::new(format!("{label}:")).color(lc).size(10.0));
                                ...
                                if let Some(size) = entry.size_bytes {
                                    ...
                                    if units.len() > 1 {
                                        let size_cycle_id = ui.id().with(("size_cycle", &entry.path));
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            let resp = ui.add(egui::Label::new(
                                                RichText::new(display).color(c.accent).size(10.0)
                                                    .underline(),
                                            ).sense(Sense::click()));
                                            if resp.clicked() {
                                                let next = (idx + 1) % units.len();
                                                ui.ctx().data_mut(|d| d.insert_persisted(size_cycle_id, next));
                                                ui.ctx().request_repaint();
                                            }
                                            if resp.hovered() {
                                                ui.ctx().set_cursor_icon(egui::CursorIcon::PointingHand);
                                ...
                                // ── Details toggle ──────────
                                ui.add_space(4.0);
                                let dtoggle = if details_open { "\u{E5CE} Details" } else { "\u{E5CF} Details" };
                                if ui.add(egui::Label::new(
                                    RichText::new(dtoggle).color(c.accent).size(10.0),
                                ).sense(Sense::click())).clicked() {
                                    let new_val = !details_open;
                                    ui.ctx().data_mut(|d| d.insert_persisted(details_id, new_val));
                                    ui.ctx().request_repaint();
                                }
    ...
    fn render_miller_empty_preview(
        &self,
        ui: &mut egui::Ui,
        width: f32,
        height: f32,
    ...
    fn render_list(&mut self, ui: &mut egui::Ui, tab_id: u64) {
        ...
        header_resp.context_menu(|ui| {
            ...
            ui.label(RichText::new("Visible columns").size(12.0).color(c.text_dim));
            ui.add(egui::Separator::default().horizontal().spacing(4.0));
        ...
        for (idx, col) in visible_cols.iter().enumerate() {
            ...
            let arrow = if current_sort.by == sort_by {
                if current_sort.ascending { " ▲" } else { " ▼" }
            } else {
                ""
            };
            ui.painter().text(
                ...
                if current_sort.by == sort_by { c.text_dim } else { c.text_muted },
            );
            ...
            if idx + 1 < visible_cols.len() {
                ...
                if sep_resp.hovered() || sep_resp.dragged() {
                    ui.ctx().set_cursor_icon(egui::CursorIcon::ResizeHorizontal);
                    ui.painter().vline(sep_rect.center().x, sep_rect.y_range(), Stroke::new(1.0, c.border));
                }
                if sep_resp.dragged() {
                    let dx = ui.ctx().input(|i| i.pointer.delta().x);
                ...
                for (i, entry) in entries.iter().enumerate() {
                    ...
                    for (idx, col) in visible_cols.iter().enumerate() {
                        ...
                        match col {
                            ...
                            ListColumn::Modified => {
                                let mod_str = entry.modified_unix_secs.map(format_modified).unwrap_or_default();
                                ui.painter().text(
                                    egui::Pos2::new(x + 2.0, y),
                                    Align2::LEFT_CENTER,
                                    mod_str,
                        ...
                        match preview.kind {
                            PreviewKind::Text => {
                                ...
                                if is_svg {
                                    ...
                                    ui.vertical_centered(|ui| {
                                        ui.add_space(8.0);
                                        if let Some(img) = self.cached_image_preview_widget(&path, max_w, max_h, 8) {
                                            ui.add(img);
                                            if !preview.body.is_empty() {
                                                ui.add_space(8.0);
                                                ui.label(RichText::new(&preview.body).color(c.text_dim).size(11.5));
                                            }
                                            return;
                                        }
                                    });
                            ...
                            PreviewKind::ArchiveMetadata => {
                                ...
                                ui.add_space(12.0);
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new(MI_FOLDER_ZIP).size(36.0).color(c.folder));
                                    ui.add_space(6.0);
                                    ui.vertical(|ui| {
                                        ui.label(RichText::new(name).color(c.text).size(14.0).strong());
                                        // First lines are summary (before "Contents:")
                                        let body = &preview.body;
                                        let (header, _listing) = if let Some(pos) = body.find("\nContents:\n") {
                                            (&body[..pos], Some(&body[pos + "\nContents:\n".len()..]))
                                        } else {
                                            (body.as_str(), None)
                                        };
                                        ui.label(RichText::new(header.trim()).color(c.text_dim).size(11.5));
                                    });
                                });
                                ...
                                if let Some(pos) = body.find("\nContents:\n") {
                                    ...
                                    if !listing.is_empty() {
                                        ...
                                        ui.separator();
                                        ui.add_space(4.0);
                                        ui.label(RichText::new("Contents").color(c.text_muted).size(11.0));
                            ...
                            PreviewKind::AudioMetadata => {
                                ...
                                ui.add_space(12.0);
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new(MI_MUSIC_NOTE).size(36.0).color(c.folder));
                                    ui.add_space(6.0);
                                    ui.vertical(|ui| {
                                        ui.label(RichText::new(name).color(c.text).size(14.0).strong());
                                    });
                                ...
                                for line in preview.body.lines() {
                                    if let Some((key, val)) = line.split_once(':') {
                                        ui.horizontal(|ui| {
                                            ui.label(RichText::new(format!("{key}:")).color(c.text_muted).size(11.5));
                                            ui.label(RichText::new(val.trim()).color(c.text).size(11.5));
                                        });
                            ...
                            PreviewKind::Pdf => {
                                ...
                                ui.add_space(12.0);
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new(MI_PICTURE_PDF).size(36.0).color(c.folder));
                                    ui.add_space(6.0);
                                    ui.vertical(|ui| {
                                        ui.label(RichText::new(name).color(c.text).size(14.0).strong());
                                    });
            ...
            .show(ctx, |ui| {
                ...
                ui.scope_builder(UiBuilder::new().max_rect(sidebar_rect), |ui| {
                    Frame::new()
                        ...
                        .show(ui, |ui| {
                            let groups: &[(&str, &[(SettingsTab, &str, &str)])] = &[
                                ("Preferences", &[
    ...
    fn render_file_row(
        ...
        if fill != Color32::TRANSPARENT {
            ...
            let stroke_col = if is_active_col { c.accent } else { c.accent_dim };
            ui.painter().rect_stroke(
                row_rect.shrink(1.0),
                3.0,
        ...
        let style = style_for_entry(
            ...
            EntryVisualState {
                ...
                hovered: response.hovered(),
                opened,
                symlink_dir: matches!(entry.kind, EntryKind::Symlink) && entry.symlink_target_is_dir == Some(true),
            },
            c,
            &self.state.config,
        );
...
impl eframe::App for OttrinApp {
    ...
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        ...
        let search_indexing =
            self.search_started && matches!(self.search_service.diagnostics().status, SearchIndexStatus::Indexing);
            ...
            egui::Area::new(egui::Id::new("resize_edge_blocker"))
                ...
                .show(ctx, |ui| {
                    ...
                    for (i, rect) in edges.iter().enumerate() {
                        ui.interact(*rect, egui::Id::new("edge").with(i), Sense::click_and_drag());
                    }
                });
        }
...
fn list_directory(path: &Path, show_hidden: bool) -> Result<Vec<FileEntry>, String> {
    ...
    let read_dir = std::fs::read_dir(path)
        .map_err(|e| format_list_dir_error(path, &e))?;
    ...
    entries.sort_by(|a, b| {
        ...
        let ord = match config.by {
            ...
            SortBy::Modified => b
                ...
                .then_with(|| a.name.to_lowercase().cmp(&b.name.to_lowercase())),
            SortBy::Kind => kind_sort_rank(a)
                .cmp(&kind_sort_rank(b))
                .then_with(|| {
...
fn render_custom_titlebar(
    ...
    let title_bar = Frame::new()
        .fill(title_fill)
        .inner_margin(egui::Margin { left: 6, right: 8, top: 1, bottom: 1 })
        .show(ui, |ui| {
            ui.set_width(ui.available_width());
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    let close_btn = ui.add(
                        egui::Button::new(RichText::new(MI_CLOSE).size(16.0).color(colors.text_muted))
                            .frame(false)
    ...
    if allow_drag {
        ...
        let can_drag = ctx.input(|i| {
            ...
            if let Some(close_r) = close_btn_rect {
                if close_r.expand(4.0).contains(pos) {
                    return false;
                }
            }
            true
...
fn file_type_description(path: &std::path::Path, kind: EntryKind) -> String {
    if kind == EntryKind::Directory { return "Folder".into(); }
    if kind == EntryKind::Symlink { return "Symbolic Link".into(); }
...
fn shift_hsva(color: Color32, hue_shift_degrees: f32, saturation_scale: f32, value_scale: f32) -> Color32 {
...
fn semantic_for_path(path: &Path) -> FileSemantic {
    ...
    classify_file(path, kind, is_executable, symlink_target_is_dir, platform_case_mode_current())
}
...
fn style_for_semantic(
    ...
    let is_neutral_category = matches!(semantic.category, FileCategory::Document | FileCategory::Unknown);
...
fn truncate_middle(name: &str, max_chars: usize) -> String {
    ...
    let tail: String = chars.iter().rev().take(right).collect::<Vec<_>>().into_iter().rev().collect();
    format!("{}…{}", head, tail)
}
...
fn theme_preset_card_with_size(
    ...
    let (rect, resp) = ui.allocate_exact_size(Vec2::new(width, card_h), Sense::click());
    let p = ui.painter();
    ...
    let thumb = Rect::from_min_size(inner.min, Vec2::new(inner.width(), thumb_h));
    ...
    p.line_segment(
        [egui::pos2(tab.left(), tab.bottom()), egui::pos2(tab.right(), tab.bottom())],
    ...
    p.text(egui::pos2(nav.left() + 10.0, nav.center().y), Align2::CENTER_CENTER, "←", FontId::proportional(8.5), preview.text_dim);
    p.text(egui::pos2(nav.left() + 24.0, nav.center().y), Align2::CENTER_CENTER, "→", FontId::proportional(8.5), preview.text_dim);
    p.text(egui::pos2(nav.left() + 38.0, nav.center().y), Align2::CENTER_CENTER, "↑", FontId::proportional(8.5), preview.text_dim);
    ...
    for (i, name) in ["Home", "Root", "Desktop"].iter().enumerate() {
        ...
        p.rect_filled(chip, preview.button_radius as f32, if i == 0 { preview.hover } else { Color32::TRANSPARENT });
        p.text(
            chip.center(),
            Align2::CENTER_CENTER,
            name,
            FontId::proportional(6.8 * preview.font_scale),
            if i == 0 { preview.text } else { preview.text_dim },
        );
    }
    ...
    for col in 0..3 {
        ...
        let col_rect = Rect::from_min_size(egui::pos2(x, content.top()), Vec2::new(col_w, content.height()));
        if col > 0 {
            p.line_segment(
                [egui::pos2(col_rect.left() - col_gap * 0.5, content.top()), egui::pos2(col_rect.left() - col_gap * 0.5, content.bottom())],
                Stroke::new(1.0, preview.border),
            );
        }
        for row in 0..4 {
            let row_y = col_rect.top() + 5.0 + row as f32 * 14.0;
            let row_rect = Rect::from_min_size(egui::pos2(col_rect.left() + 2.0, row_y), Vec2::new(col_rect.width() - 4.0, 10.0));
            ...
            p.rect_filled(
                Rect::from_min_size(egui::pos2(row_rect.left() + 3.0, row_rect.center().y - 2.0), Vec2::new(6.0, 4.0)),
                2.0,
                preview.folder,
            );
            p.text(
                ...
                Align2::LEFT_CENTER,
                if row == 1 && col == 2 { "google..." } else { "folder" },
                FontId::proportional(6.8 * preview.font_scale),
                if col == 2 && row == 1 { preview.file } else { preview.text_dim },
            );
        }
    }
    ...
    for i in 0..3 {
        let icon = Rect::from_min_size(egui::pos2(smart.left() + 3.0, smart.top() + 6.0 + i as f32 * 15.0), Vec2::new(10.0, 10.0));
        p.rect_filled(icon, preview.button_radius as f32, if i == 0 { preview.selected_bg } else { preview.hover });
    }
    ...
    p.text(
        ...
        title,
        FontId::proportional(12.0),
        if selected { app_colors.text } else { app_colors.text_dim },
    );
...
fn paint_theme_tuning_track(ui: &mut egui::Ui, rect: Rect, title: &str, tint: Color32, c: Colors) {
    ...
    if title == "Contrast" {
        ...
        for i in 0..steps {
            ...
            painter.rect_filled(
                Rect::from_min_max(egui::pos2(x0, top.top() + 1.0), egui::pos2(x1, top.bottom() - 1.0)),
                0.0,
                top_col,
            );
            painter.rect_filled(
                Rect::from_min_max(egui::pos2(x0, bottom.top() + 1.0), egui::pos2(x1, bottom.bottom() - 1.0)),
                0.0,
                bottom_col,
            );
        }
    ...
    let stops: Vec<Color32> = match title {
        ...
        "Brightness" => vec![Color32::from_rgb(10, 10, 12), tint, mix_color(Color32::WHITE, tint, 0.18)],
        _ => vec![mix_color(c.panel_raised, c.border, 0.25), mix_color(c.panel_raised, c.border, 0.85)],
    };
    for i in 0..steps {
        let t0 = i as f32 / steps as f32;
        let t1 = (i + 1) as f32 / steps as f32;
...
fn search_scope_summary(scope: SearchScope, current_dir: &Path) -> String {
    match scope {
        ...
        SearchScope::CurrentFolder => format!("Scope: Current folder ({})", compact_search_path(current_dir)),
    }
}
...
fn search_root_summary(diagnostics: &SearchIndexDiagnostics) -> String {
    format!(
        ...
        diagnostics.configured_root_count,
        diagnostics.watched_root_count
    )
}
...
fn search_diagnostics_tooltip(
    ...
    if let Some(active_root) = diagnostics.active_root.as_ref() {
        lines.push(format!("Current root: {}", compact_search_path(active_root)));
    }
    if let Some(detail) = diagnostics.detail.as_ref().filter(|d| !d.trim().is_empty()) {
        lines.push(detail.trim().to_string());
    }
    if let Some(err) = diagnostics.last_error.as_ref().filter(|e| !e.trim().is_empty()) {
...
fn theme_preview_strip(ui: &mut egui::Ui, preview: Colors, width: f32, height: f32, c: Colors) {
    ...
    for (idx, swatch) in swatches.into_iter().enumerate() {
        ...
        let swatch_rect = Rect::from_min_max(egui::pos2(x0, rect.top()), egui::pos2(x1, rect.bottom()));
...
mod tests {
    ...
    fn size_sort_is_fixed_descending() {
        ...
        sort_entries(&mut entries, &ottrin_core::SortConfig { by: SortBy::Size, ascending: true });
        assert_eq!(entries[0].name, "large.txt");
        sort_entries(&mut entries, &ottrin_core::SortConfig { by: SortBy::Size, ascending: false });
        assert_eq!(entries[0].name, "large.txt");
    }
    ...
    fn kind_sort_keeps_group_order_and_flips_names_within_group() {
        ...
        sort_entries(&mut entries, &ottrin_core::SortConfig { by: SortBy::Kind, ascending: true });
        assert_eq!(entries[0].name, "docs");
        assert_eq!(entries[1].name, "alpha.txt");
        assert_eq!(entries[2].name, "beta.txt");

        sort_entries(&mut entries, &ottrin_core::SortConfig { by: SortBy::Kind, ascending: false });
        assert_eq!(entries[0].name, "docs");
